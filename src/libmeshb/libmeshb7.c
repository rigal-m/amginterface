...
int GmfStatMesh(int64_t MshIdx, int *ver, int *dim, int *mod)
{
	GmfMshSct *msh;


  msh = (GmfMshSct *) MshIdx;
  if ( ! msh ) return 0;
   ...
   if(msh->typ & Asc)
   {
      for(i=0;i<kwd->SolSiz;i++)
      {
         if(kwd->fmt[i] == 'r')
            ...
            safe_fscanf(msh->hdl, "%ld", &IntTab[ (*IntCpt)++ ], msh->err);
         else if(kwd->fmt[i] == 'c')
         {
   ...
   if(msh->typ & Asc)
   {
      for(i=0;i<kwd->SolSiz;i++)
      {
         if(kwd->fmt[i] == 'r')
            ...
            fprintf(msh->hdl, "%ld ", LngTab[ LngCpt++ ]);
         else if(kwd->fmt[i] == 'c')
